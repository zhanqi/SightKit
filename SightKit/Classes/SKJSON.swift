...
public enum SKJSONError: Int, Swift.Error {
...
extension SKJSONError: CustomNSError {

    /// return the error domain of SwiftyJSONError
    public static var errorDomain: String { return "com.sightkit.SKJSON" }
...
public struct SKJSON {
    ...
    */
     ...
     */
    public mutating func merge(with other: SKJSON) throws {
     ...
     */
    public func merged(with other: SKJSON) throws -> SKJSON {
        var merged = self
     ...
     */
     fileprivate mutating func merge(with other: SKJSON, typecheck: Bool) throws {
        if type == other.type {
            switch type {
            case .dictionary:
                for (key, _) in other {
            ...
            case .array:
                self = SKJSON(arrayValue + other.arrayValue)
            default:
                self = other
            }
        } else {
            if typecheck {
                throw SKJSONError.wrongType
            } else {
                self = other
            }
        }
    ...
    public fileprivate(set) var error: SKJSONError?
    ...
    public var object: Any {
        ...
        set {
            ...
            default:
                type = .unknown
                error = SKJSONError.unsupportedType
            }
        }
    }
    ...
    public static var nullJSON: SKJSON { return null }
    public static var null: SKJSON { return SKJSON(NSNull()) }
}

/// Private method to unwarp an object recursively
private func unwrap(_ object: Any) -> Any {
    switch object {
    case let json as SKJSON:
...
public typealias JSONIndex = Index<SKJSON>
public typealias JSONRawIndex = Index<Any>

extension SKJSON: Swift.Collection {
    ...
    public subscript (position: Index) -> (String, SKJSON) {
        switch position {
        case .array(let idx):      return (String(idx), SKJSON(rawArray[idx]))
        case .dictionary(let idx): return (rawDictionary[idx].key, SKJSON(rawDictionary[idx].value))
        default:                   return ("", SKJSON.null)
        }
    }
}
...
extension SKJSON {
    ...
    fileprivate subscript(index index: Int) -> SKJSON {
        get {
            if type != .array {
                var r = SKJSON.null
                r.error = self.error ?? SKJSONError.wrongType
                return r
            } else if rawArray.indices.contains(index) {
                return SKJSON(rawArray[index])
            } else {
                var r = SKJSON.null
                r.error = SKJSONError.indexOutOfBounds
                return r
            }
        }
        set {
    ...
    fileprivate subscript(key key: String) -> SKJSON {
        get {
            var r = SKJSON.null
            if type == .dictionary {
    ...
    fileprivate subscript(sub sub: JSONSubscriptType) -> SKJSON {
        get {
            switch sub.jsonKey {
     ...
     */
    public subscript(path: [JSONSubscriptType]) -> SKJSON {
        get {
...
extension SKJSON: Swift.ExpressibleByStringLiteral {
...
extension SKJSON: Swift.ExpressibleByIntegerLiteral {
...
extension SKJSON: Swift.ExpressibleByBooleanLiteral {
...
extension SKJSON: Swift.ExpressibleByFloatLiteral {
...
extension SKJSON: Swift.ExpressibleByDictionaryLiteral {
...
extension SKJSON: Swift.ExpressibleByArrayLiteral {
...
extension SKJSON: Swift.RawRepresentable {

    public init?(rawValue: Any) {
        if SKJSON(rawValue).type == .unknown {
            return nil
        } else {
            self.init(rawValue)
        }
    ...
    fileprivate func _rawString(_ encoding: String.Encoding = .utf8, options: [writingOptionsKeys: Any], maxObjectDepth: Int = 10) throws -> String? {
        guard maxObjectDepth > 0 else { throw SKJSONError.invalidJSON }
        switch type {
        case .dictionary:
            do {
...
extension SKJSON: Swift.CustomStringConvertible, Swift.CustomDebugStringConvertible {
...
extension SKJSON {
    ...
    //Optional [JSON]
    public var array: [SKJSON]? {
        return type == .array ? rawArray.map { SKJSON($0) } : nil
    }

    //Non-optional [JSON]
    public var arrayValue: [SKJSON] {
        return self.array ?? []
    }
...
extension SKJSON {
    ...
    public var dictionary: [String: SKJSON]? {
        if type == .dictionary {
            var d = [String: SKJSON](minimumCapacity: rawDictionary.count)
            rawDictionary.forEach { pair in
                d[pair.key] = SKJSON(pair.value)
            }
            return d
        } else {
            return nil
    ...
    public var dictionaryValue: [String: SKJSON] {
        return dictionary ?? [:]
    }
...
extension SKJSON: Swift.Comparable {}

public func == (lhs: SKJSON, rhs: SKJSON) -> Bool {
...
extension SKJSON: Codable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        if let container = try? decoder.singleValueContainer(), !container.decodeNil() {
            for type in SKJSON.codableTypes {
                if object != nil {
                    break
                }
                // try to decode value
                ...
                case let jsonValueArrayType as [SKJSON].Type:
                    object = try? container.decode(jsonValueArrayType)
                case let jsonValueDictType as [String: SKJSON].Type:
