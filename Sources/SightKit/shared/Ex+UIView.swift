...
public extension NSView {
    ...
    @discardableResult func wBgColor(_ color:NSColor?) -> Self{
        self.backgroundColor = color
        return self
    }
    @discardableResult func wFrame(_ frame:CGRect) -> Self{
    ...
    @discardableResult func addTo(_ fatherView:NSView) -> Self{
        fatherView.addSubview(self)
        return self
    }
}
...
public extension NSView{
    ...
    /// 设置边
    @discardableResult func border(width:CGFloat,color:NSColor) -> Self{
        self.layer?.masksToBounds = true
        self.layer?.borderWidth = width
        self.layer?.borderColor = color.cgColor
        return self
    ...
    /// 设置圆角和阴影 make sure the view's backgroundColor is not clear color, then this method will work.
    @discardableResult func corner(radius:CGFloat,shadowColor:NSColor,shadowOffset:CGSize,shadowOpacity:Float,shadowRadius:CGFloat) -> Self {
        layer?.cornerRadius = radius
        layer?.shadowColor = shadowColor.cgColor;
        layer?.shadowOffset = shadowOffset //(0,0)时是四周都有阴影
        layer?.shadowOpacity = shadowOpacity;
    ...
    ///   - offset: shadow offset (default is .zero).
    ///   - opacity: shadow opacity (default is 0.5).
    func addShadow(ofColor color: NSColor = NSColor(red: 0.12, green: 0.12, blue: 0.12, alpha: 0.5), radius: CGFloat = 5, offset: CGSize = CGSize(width: 2, height: 2), opacity: Float = 0.3) {
        layer?.shadowColor = color.cgColor
        layer?.shadowOffset = offset
        layer?.shadowRadius = radius
        layer?.shadowOpacity = opacity
...
public extension NSView {
    ...
    var viewController: NSViewController? {
        var parent: NSResponder? = self
        while parent != nil {
            parent = parent?.nextResponder
            if let viewController = parent as? NSViewController {
                return viewController
            }
        }
        return nil
...
public extension NSView {
    ...
    ///   - viewEnum: closure to config the subview
    /// - Returns: self
    func divideHorizontal(num:Int,viewEnum:((_ subView:NSView,_ index:Int)->())){
        guard num > 0 else {
            return
        }
        
        for i in 0...num-1 {
            let view = NSView().addTo(self).csFullfillVertical().csHorizonSeparate(num: CGFloat(num), index: Int(i))
            viewEnum(view,i)
        }
    }
    /// divide the view  vertically equally with several subviews
    ...
    ///   - viewEnum: closure to config the subview
    /// - Returns: self
    func divideVertical(num:Int,viewEnum:((_ subView:NSView,_ index:Int)->())){
        guard num > 0 else {
            return
        }
        
        for i in 0...num-1 {
            let view = NSView().addTo(self).csFullfillHorizontal().csVerticalSeparate(num: CGFloat(num), index: Int(i))
            viewEnum(view,i)
        }
    }
}
...
public extension NSView {
    private struct SKAddViewProperties{
        static var lastSKAddView:NSView? = nil
    }
    
    var lastSKAddView:NSView?{
        get {
            return objc_getAssociatedObject(self, &SKAddViewProperties.lastSKAddView) as? NSView
        }
        set {
            if let unwrappedValue = newValue {
                objc_setAssociatedObject(self, &SKAddViewProperties.lastSKAddView, unwrappedValue as NSView?, .OBJC_ASSOCIATION_RETAIN_NONATOMIC)
            }
        }
    }
    ...
    /// 横向填充一个view，宽度由内部决定，或在block中配置
    @discardableResult func addSubviewHorizontal(viewConfig:((_ subView:NSView)->())) -> NSView? {
        let view = NSView()
        if let lastV = self.lastSKAddView {
            view.addTo(self).cstoRightOf(view: lastV).csFullfillVertical().csRightLessThanOrEqual()
        }else{
    ...
    /// 纵向填充一个view，高度由内部决定，或在block中配置
    @discardableResult func addSubviewVertical(viewConfig:((_ subView:NSView)->())) -> NSView? {
        let view = NSView()
        if let lastV = self.lastSKAddView {
            view.addTo(self).cstoBottomOf(view: lastV).csFullfillHorizontal().csBottomLessThanOrEqual()
        }else{
    ...
    /// 横向填充一个view
    @discardableResult func addSubviewHorizontalSpace(width:CGFloat) -> NSView? {
        let view = NSView()
        if let lastV = self.lastSKAddView {
            view.addTo(self).cstoRightOf(view: lastV).csWidth(width).csFullfillVertical().csRightLessThanOrEqual()
        }else{
    ...
    /// 纵向填充一个view
    @discardableResult func addSubviewVerticalSpace(height:CGFloat) -> NSView? {
        let view = NSView()
        if let lastV = self.lastSKAddView {
            view.addTo(self).cstoBottomOf(view: lastV).csHeight(height).csFullfillHorizontal().csBottomLessThanOrEqual()
        }else{
    ...
    /// 横向填充多个view，宽度由内部决定，或在block中配置
    @discardableResult func addSubViewHorizontalRepeat(num:UInt,viewConfig:(_ subView:NSView,_ index:UInt)->()) -> NSView?{
        for i in 0..<num {
            addSubviewHorizontal { (v) in
                viewConfig(v,i)
            }
    ...
    /// 纵向填充多个view，高度由内部决定，或在block中配置
    @discardableResult func addSubViewVerticalRepeat(num:UInt,viewConfig:(_ subView:NSView,_ index:UInt)->()) -> NSView?{
        for i in 0..<num {
            addSubviewVertical(viewConfig: { (v) in
                viewConfig(v,i)
            })
...
public extension NSView {
    /** 添加边线
     ...
     view.addLine(edges: .right,.top,.bottom, color: .lightGray, value: 0.5)
     */
    @discardableResult func addLine(edges:SKViewEdge...,color:NSColor,value:Float) -> Self{
        for edge in edges {
            let line = NSView().wBgColor(color).addTo(self)
